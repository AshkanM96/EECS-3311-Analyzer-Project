...
feature {NONE} -- Initialization
	...
	user_create_interface_objects
		...
		do
			create {ETF_MODEL_FACADE} model_obj.make
			create {PLAIN_TEXT_FILE} session_file.make_with_name (session_file_path)
			output.insert_text (model_obj.model.out)
			status.insert_text ("List of commands: %N")
			status.insert_text ("================= %N")
			status.insert_text (model_obj.man_page)
		end
...
feature {NONE} -- Implementation
	...
	load_pressed
		...
		local
			fu: ETF_FILE_UTILITY
		do
			create {ETF_FILE_UTILITY} fu.read_text_from (session_file_path)
			if fu.error then
				clear_display
				status.insert_text (fu.error_string)
			else
				if (not model_obj.history.is_empty) then
					reset_pressed
				end
...
feature {NONE}
	...
	execute_command (cmd: STRING)
		...
		local
			...
			error: BOOLEAN
		do
			create {STRING} problem.make_empty
			if (not error) then
				model_obj.execute_cmd (cmd)
				clear_display
				display_history (model_obj.history)
				status.insert_text (model_obj.status_message)
				output.insert_text (model_obj.output_message)

					-- in case there's a syntax or type error,
					-- keep the typed command on the text field
					-- so that it can be fixed.
				if model_obj.error then
